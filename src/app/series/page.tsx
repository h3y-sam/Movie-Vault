      ...
      {showFilters && (
        <div className="max-w-7xl mx-auto px-6 md:px-10 mb-8 animate-fade-in">
          <div className="bg-[#13131a]/90 border border-white/5 rounded-xl p-5 grid grid-cols-1 sm:grid-cols-4 gap-6 backdrop-blur-md">
            ...
            {/* Release Year Input */}
            <div className="space-y-2">
              <label className="text-[10px] text-[#6b7280] font-black uppercase tracking-widest">First Aired Year</label>
              <input
                type="number"
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6">
              {filteredSeries.map((series, index) => (
                <MovieCard key={`series-${series.id}-${index}`} item={series} index={index} />
              ))}
            </div>
